...
frappe.ui.form.on(itemDoctypName, {
	...
	product_type_name: function (frm) {
		frm.set_value("is_stock_item", frm.doc.product_type_name !== "Service" ? 1 : 0);
	},
});

async function getEtimsSettings(frm) {
	const { message: data } = await frappe.call({
		method: "kenya_compliance_via_digitax.kenya_compliance_via_digitax.utils.get_etims_action_data",
		args: {
			doctype: frm.doctype,
...
async function applyEtimsAutofillFields(frm) {
	...
	const fallbackSetting = frm.etims?.allSettings?.length ? frm.etims.allSettings[0].name : null;

	const { message: values } = await frappe.call({
		method: "kenya_compliance_via_digitax.kenya_compliance_via_digitax.overrides.server.item.autofill_item_etims_fields",
		args: {
...
function toggleImportedLocks(frm) {
	if (frm.doc.imported_item_submitted) {
		frm.toggle_enable("referenced_imported_item", false);
		frm.toggle_enable("imported_item_status", false);
	}
}
...
function setupButtons(frm) {
	...
	const canRegister =
		!frm.doc.prevent_etims_registration &&
		!frm.doc.item_registered &&
...
function showCompanySelectionModal(frm, actionType, availableSettings) {
	if (!availableSettings.length) {
		frappe.msgprint(
			__("No available eTIMS settings for this action. Please check configuration."),
		);
		return;
	}
...
function executeItemAction(frm, actionType, settingName) {
	...
	switch (actionType) {
		...
		case "update_item":
			method =
				"kenya_compliance_via_digitax.kenya_compliance_via_digitax.apis.apis.perform_item_registration";
			args = {
		...
		case "fetch_item_details":
			method =
				"kenya_compliance_via_digitax.kenya_compliance_via_digitax.apis.apis.fetch_item_details";
			args = {
				settings_name: settingName,
				request_data: {
		...
		case "submit_inventory":
			method =
				"kenya_compliance_via_digitax.kenya_compliance_via_digitax.apis.apis.submit_inventory";
			args = {
				name: frm.doc.name,
				settings_name: settingName,
			};
