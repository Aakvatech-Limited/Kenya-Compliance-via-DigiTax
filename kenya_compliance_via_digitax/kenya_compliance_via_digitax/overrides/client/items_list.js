...
frappe.listview_settings[doctypeName].onload = async function (listview) {
	...
	listview.page.add_action_item(__("Bulk Register Items"), function () {
		const itemsToRegister = listview.get_checked_items().map((item) => item.name);
		if (!itemsToRegister.length) {
			frappe.msgprint(__("Please select items to register"));
			return;
		}

		showSettingsModalAndExecute("Bulk Register Items", allSettings, (settings_name) => ({
...
function addItemListActions(listview, allSettings) {
	...
	actions.forEach((action) => {
		listview.page.add_inner_button(
			action.label,
			function () {
				showSettingsModalAndExecute(action.label, allSettings, (settings_name) =>
					action.getCallArgs(settings_name),
				);
			},
			__("eTims Actions"),
		);
...
function executeEtimsAction(method, args, successMsg) {
	frappe.call({
		method: `kenya_compliance_via_digitax.kenya_compliance_via_digitax.apis.apis.${method}`,
		args: args,
		freeze: true,
