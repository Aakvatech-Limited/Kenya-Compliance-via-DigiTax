...
def generic_invoices_on_submit_override(
    ...
    customer_digitax_id = get_digitax_id("Customer", doc.customer, settings_doc.name)
    if not customer_digitax_id:
        frappe.msgprint(
    ...
    for item in doc.items:
        ...
        digitax_id = get_digitax_id("Item", item_doc.get("name"), settings_doc.name)
        if not digitax_id:
    ...
    if doc.is_return:
        ...
        if not return_invoice.successfully_submitted:
            frappe.msgprint(
